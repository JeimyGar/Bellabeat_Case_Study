...
GROUP BY 1,2;
```
This last query is to see the total minutes of each activity level. I calculated the total number of minutes for each activity level and gave it an alias. 
```SQL
...
4. In this scatterplot titled "Step Count vs. BMI" we can see the relationship between body mass index (BMI) and step count. We can see a negative correlation between BMI and step count. This would lead us to believe that as BMI increases, step count tends to decrease. There are some outliers in this data.
